...
int main(int argc, char* argv[] ) {
	...
	if(-1 == uart_open(portname, B115200, 1)) 
		return -1;

	unsigned char ch[100]={0};
	unsigned int status, j, last=0;
	while (1) {
		for (j=0; j<20; j++) ch[j]=0;
		last=0;
		printf("[+] Reading string from UART.\n");
		 while ( ch[last] != 0xa) {
			status=uart_read(&ch[0],10);
			if (status == -1) { printf("error=%x uart fd = %x, status = %d\n",errno, uartFd,status); exit(-1); }
			for(j=0; j< status; j++) { printf(" %c", ch[j]); last=j; } //printf("\n");
		}
	}
